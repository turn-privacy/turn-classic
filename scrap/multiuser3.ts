...
const showOperatorUtxos = async () => {
  const operatorUtxos = await lucid.utxosAt(operator.address);
  const amounts = operatorUtxos
    .map((utxo) => utxo.assets.lovelace)
    .map((amount) => (Number(amount) / Number(1_000_000n)).toFixed(6));
  console.log(`Operator UTxOs --> `, amounts);
};
...
The participants are sending funds to the recipient, themselves, and the operator.

The operator is not providing ANY inputs.

So where is the network fee coming from?

Somehow it seems to be comming out of the operator's fee (which is what I want) but I don't understand why or how!

Shouldn't I have to calculate the network fee and subtract it from the operator's fee?
...
// const assembled = completeTx.assemble([aliceWitness, bobWitness, charlieWitness]);
